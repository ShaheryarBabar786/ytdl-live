...
export class ComponentsComponent implements OnInit, OnDestroy {
  ...
  onInputChanged() {
    ...
    if (this.videoURL && this.videoURL.trim() !== "") {
      ...
      setTimeout(async () => {
        try {
          const videoSize = await this.getYTVideoSize(this.videoURL);
          console.log('Video Size:', videoSize); // Display or use the video size here
          // Other logic related to video size...
        } catch (error) {
          console.error('Error fetching video size:', error);
          this.errorMessage = "Error fetching video size. Please try again.";
        } finally {
          this.loading = false;
        }
      }, 2000);
  ...
  async getYTVideoSize(videoURL: string): Promise<number> {
    try {
      const response = await this.http.head(videoURL, { observe: 'response' }).toPromise();
