...
router.post('/downloadvideomp4', async (req, res) => {
    // Capture start time when request is received
    ...
    try {
        ...
        // Capture end time after processing but before starting video streaming
        const processingEndTime = process.hrtime(requestStartTime);
        const processingTime = processingEndTime[0] + processingEndTime[1] / 1e9; // Convert to seconds
        console.log(`Processing time on server: ${processingTime} seconds`);

        // Set response headers and prepare for streaming
        ...
        const videoStream = ytdl(videoURL, { format, quality: 'highestaudio' });
        videoStream.pipe(res);
