...
router.get('/videoSize', async (req, res) => {
  ...
  try {
    ...
    const info = await ytdl.getInfo(videoURL);
    const contentLengthBytes = parseInt(info.videoDetails.lengthSeconds, 10); // Assuming lengthSeconds is in bytes
    const contentLengthMB = contentLengthBytes / (1024 * 1024); // Convert bytes to megabytes

    res.status(200).json({ videoSizeMB: contentLengthMB.toFixed(2) }); // Limit to 2 decimal places
  } catch (error) {
