...
router.post('/downloadvideomp4', async (req, res) => {
    ...
    try {
        ...
        const fileSize = format.contentLength || 0; // Content length in bytes

        res.setHeader('Content-Disposition', `attachment; filename="${sanitizeTitle(info.videoDetails.title)}.mp4"`);
        res.setHeader('Content-Type', 'video/mp4');
        res.setHeader('Content-Length', fileSize); // Set Content-Length header with file size

        console.log('File size:', fileSize); // Log the file size

        // Send video information along with file size
