...
router.post('/downloadvideomp4', async (req, res) => {
    ...
    try {
        ...
        videoStream.on('end', () => {
            ...
            const responseTime = endTime[0] + endTime[1] / 1e9; // Convert to seconds
            console.log(`Video download completed successfully in ${responseTime} seconds.`);

            // Send response with response time
            res.status(200).send({ message: 'Video download completed successfully.', responseTime: `${responseTime} seconds` });
        });
